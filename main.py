...
def open_home():
    def create_database_if_not_found():
        ...
        def create_account_table():
            ...
            c = sql.connect(host="localhost", database='expense_manager', user="root", passwd="revant")
            c_cur = c.cursor()
            c_cur.execute(
        ...
        s = sql.connect(host="localhost", user="root", passwd="revant")
        cur = s.cursor()
        ...
        def act():
            ...
            c_cur.execute(
                "create table {}(SR INT PRIMARY KEY AUTO_INCREMENT,TITLE VARCHAR(100),TYPE VARCHAR(20),SENT_TO_RECIEVED_FROM VARCHAR(100),COST INT,d_ate DATE)".format(
                    name))     #it will create the transaction table for every user
            c.commit()
            c_cur.close()
            c.close()
            fetch_acc_list()
    ...
    def fetch_acc_list():
        ...
        s = sql.connect(host="localhost", user="root", passwd="revant", database="expense_manager")
        s_cur = s.cursor()
...
def open_dashboard(j):
    ...
    Type = Label(info, text=j[3], bg="#F2CC8F", font=("Arial", 20))
    Type.grid(row=1, column=0, padx=10, pady=15, sticky="W")

    salary = Label(info, text=j[4], bg="#F2CC8F", font=("Arial", 20))
